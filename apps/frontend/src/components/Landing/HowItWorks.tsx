  ...
  return (
    <section id="how-it-works" className="py-32 relative">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[400px] rounded-full bg-primary/5 blur-3xl" />
      </div>
            ...
            return (
              <div key={step.step} className="relative flex flex-col">
                ...
                <div className="flex flex-col items-center mb-8">
                  <div className="relative z-10 w-16 h-16 rounded-xl bg-card border border-border/50 shadow-sm flex items-center justify-center mb-3 hover:border-primary/60 hover:bg-primary/5 transition-all group">
                    <Icon className="w-7 h-7 text-primary group-hover:scale-110 transition-transform" />
                    <span className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-primary text-primary-foreground text-[10px] font-bold flex items-center justify-center shadow-sm">
                      {i + 1}
                    </span>
                  </div>
                </div>
                ...
                {/* Content */}
                <h3 className="text-foreground font-semibold text-lg mb-2 text-center">{step.title}</h3>
                ...
                <div className="mt-auto flex flex-wrap gap-1.5 justify-center">
                  {step.detail.split(' · ').map((tag) => (
                    <Badge
                      key={tag}
                      variant="secondary"
                      className="text-[11px] bg-secondary/30 text-muted-foreground border-border/50 px-2 py-0.5 hover:bg-secondary/60"
                    >
                      {tag}
                    </Badge>
                  ))}
